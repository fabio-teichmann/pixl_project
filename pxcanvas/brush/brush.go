...
func Cursor(config apptype.PxCanvasConfig, brush apptype.BrushType, ev *desktop.MouseEvent, x int, y int) []fyne.CanvasObject {
	...
	case brush == Pixel:
		...
		right.StrokeWidth = 3
		right.Position1 = fyne.NewPos(xOrigin+pxSize, yOrigin+pxSize)
		right.Position2 = fyne.NewPos(xOrigin+pxSize, yOrigin)

		top := canvas.NewLine(cursorColor)
		top.StrokeWidth = 3
		top.Position1 = fyne.NewPos(xOrigin+pxSize, yOrigin)
		top.Position2 = fyne.NewPos(xOrigin, yOrigin)

		objects = append(objects, left, bottom, right, top)
	}
	return objects
}
