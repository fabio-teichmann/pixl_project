...
type SwatchRenderer struct {
	...
	parent  *Swatch // to determine whether or not something was selected
}
...
func (renderer *SwatchRenderer) Layout(size fyne.Size) {
	renderer.objects[0].Resize(size) // resizes existing square to maximum space available
}
...
func (renderer *SwatchRenderer) Refresh() {
	...
	if renderer.parent.Selected {
		...
		// color upon select
		renderer.square.StrokeColor = color.NRGBA{255, 0, 255, 255}
